(() => {
  ...
  list.addEventListener('click', async event => { const openIndex = event.target.dataset.open; if (openIndex !== undefined) { const tab = window.open('', '_blank'); if (tab) tab.opener = null; try { const result = await request({action:'admin-open',id:reports[Number(openIndex)].id}); if (tab) tab.location.href = result.reportUrl; else setStatus('Please allow popups to open the report.', true); } catch (error) { tab?.close(); setStatus(error.message, true); } return; } const index = event.target.dataset.delete; if (index === undefined) return; const item = reports[Number(index)]; if (!confirm(`Remove the lab report for ${item.batchNumber}? This will permanently remove its stored document.`)) return; try { await request({action:'admin-delete',id:item.id}); await load(); setStatus('Lab report removed from batch lookup.'); } catch (error) { setStatus(error.message || 'Could not remove report.', true); } });
